...
const requireAdmin = (req, res, next) => {
    try {
        if (!req.user) return res.status(401).json({ message: "Unauthorized.", status: "Error" })
        if (req.user.role?.code !== "ADMIN")
            return res.status(403).json({ message: "Admin access required.", status: "Error" })
        next()
    } catch (error) {
...
module.exports = { authentication, checkPermission, requireAdmin }